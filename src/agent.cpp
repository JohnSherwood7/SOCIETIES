 ...
 *    renewalRate, steepness, scaling, minResEff, maxResEff, maxResExp, minDevEff,
 *    maxDevEff, maxDevExp, lifetime, resTradePower, devTradePower, patent, groupNumber.
 * amount of resources is stored in RES_INIT_AMOUNT[] vector, defined in globals.cpp.
 */
void Agent::defineProperties()
{
     ...
     */
    resTradePower = personalValues[0][11];
    devTradePower = personalValues[0][12];
    inventSpeed = personalValues[0][13];
    group = personalValues[0][15];
    for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
        ResProperties newResProp = ResProperties();
        newResProp.steepness = personalValues[resId][1];
        newResProp.scaling = personalValues[resId][2];

        newResProp.calcMarginalUtilities();

        newResProp.minResEffort = personalValues[resId][4];

        newResProp.maxResEffort = personalValues[resId][5];
        newResProp.maxResExperience = personalValues[resId][6];
        newResProp.calcResEfforts();
        
        newResProp.averageLifetime = personalValues[resId][10];

        newResProp.renewalRate = personalValues[resId][3] / (double)100; // Use percent for renewalRate
    ...
    for (int devnum = 0; devnum < NUM_DEVICE_TYPES; devnum++) {
        ...
        for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
            ...
            newDevProp.minDeviceEffort = personalValues[resId][7];
            newDevProp.maxDeviceEffort = personalValues[resId][8];
            newDevProp.maxDeviceExperience = personalValues[resId][9];
...
void Agent::workDay()
{
    ...
    // Creates a list of marginal utilities per effort of each available resource
    /*
    ...
    for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
        if (glob.res[resId].inSimulation) {
            // Only extract the resources that are unlimited or sufficient
            if ( (glob.RES_AMOUNT[resId].first == false) || (glob.RES_AMOUNT[resId].second > glob.MIN_RES_AMOUNT) ) {
                device_name_t bestDev = bestDevice(resId);
                if (bestDev != NO_DEVICE) {
                    valuePerEfforts.push_back(utilPerEffort(resId) * getDeviceFactor(bestDev));
                } else {
    ...
    while (time < glob.DAY_LENGTH) {
        ...
        /*
         * If maxUtilIndexes is empty,
         * consider extracting from any unlimited or sufficient resources.
         */
        if (maxUtilIndexes.empty()) {
            for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
                if ( (glob.RES_AMOUNT[resId].first == false) || (glob.RES_AMOUNT[resId].second >= glob.MIN_RES_AMOUNT) ) {
                    maxUtilIndexes.push_back(resId);
                }
        ...
        /*
         * If maxUtilIndexes is still empty, it means all resources are used up.
         * Agents stop working/extracting.
         */
        if (maxUtilIndexes.empty()) {
            break;
        }
        ...
        double timeChange;
        if (bestDev != NO_DEVICE) {
            timeChange = effortCalc(workIndex) / getDeviceFactor(bestDev);
        ...
        if (nextBestDevice != NO_DEVICE) {
            valuePerEfforts[workIndex] = utilPerEffort(workIndex) * getDeviceFactor(nextBestDevice);
...
void Agent::toolInvention()
{
     ...
     * and the resources are not collapsed.
     * Since consideredResources is from heldResources, 
     * then no tool is made of, or for, a "collapsed" resource.
     */
    vector<int> heldResources;
    for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
        vector<int>::iterator idx = find(glob.nonDepletedRes.begin(), glob.nonDepletedRes.end(), resId);
        if (idx != glob.nonDepletedRes.end()) {
            ...
            experienceCheck.push_back(0.0);
        }
    }
    /*
     * If the agent has enough resources in heldResources to get to RESOURCES_IN_TOOL + 1 resources, 
     * the agent proceeds to choose two resources that it has most experience in.
     */
    if (heldResources.size() > glob.RESOURCES_IN_TOOL) {
    /*
     ...
     */
        ...
        /* If the number of considered resources < RESOURCES_IN_TOOL,
         * agents cannot invent tools.
         */
        if ( (int) consideredResources.size() >= glob.RESOURCES_IN_TOOL) {
            /*
            ...
            if (! noTool.empty()) {
                ...
                if ( ran < probability ) {
                    ...
                    /*
                     * add toolUse (resId the tool is used to extract) to toolsKnown and remove it from toolsNotKnown.
                     */
                    vector<int>::iterator idx = find(glob.toolsNotKnown.begin(), glob.toolsNotKnown.end(), toolUse);
                    if (idx != glob.toolsNotKnown.end()) {
                        glob.toolsNotKnown.erase(idx);
                        glob.toolsKnown.push_back(toolUse);
                    }
                    ...
                    LOG(3) << "Tool for " << toolUse << " has been invented and is composed of " << glob.discoveredDevices[TOOL][toolUse]->componentsAsString() << ".";
                }
            }
...
void Agent::deviceInvention(device_name_t device, device_name_t componentType)
{
    ...
    for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
        ...
        if ( (devProp[componentType][resId].getDeviceExperience() > 0) && 
             (glob.res[resId].inSimulation) && 
             (glob.discoveredDevices[componentType][resId] != NULL) ){
    ...
    if (! noDevice.empty()) {
        ...
        if (glob.random_01() < (glob.DEVICE_PROBABILITY_FACTOR * sumExp * inventSpeed)) {
            ...
            case MACHINE:
                 ...
                 * add toolUse (resId the tool is used to extract) to toolsKnown and remove it from toolsNotKnown.
                 */
                idx = find(glob.machinesNotKnown.begin(), glob.machinesNotKnown.end(), deviceUse);
                if (idx != glob.machinesNotKnown.end()) {
                    glob.machinesNotKnown.erase(idx);
                    glob.machinesKnown.push_back(deviceUse);
                }
                break;
            case FACTORY:
                glob.discoveredDevices[device][deviceUse] =
                 ...
                 * add toolUse (resId the tool is used to extract) to toolsKnown and remove it from toolsNotKnown.
                 */
                idx = find(glob.factoriesNotKnown.begin(), glob.factoriesNotKnown.end(), deviceUse);
                if (idx != glob.factoriesNotKnown.end()) {
                    glob.factoriesNotKnown.erase(idx);
                    glob.factoriesKnown.push_back(deviceUse);
                }
                break;
            case DEVMACHINE:
                glob.discoveredDevices[device][deviceUse] =
                 ...
                 * add toolUse (resId the tool is used to extract) to toolsKnown and remove it from toolsNotKnown.
                 */
                idx = find(glob.devMachinesNotKnown.begin(), glob.devMachinesNotKnown.end(), deviceUse);
                if (idx != glob.devMachinesNotKnown.end()) {
                    glob.devMachinesNotKnown.erase(idx);
                    glob.devMachinesKnown.push_back(deviceUse);
                }
                break;
            case INDUSTRY:
                glob.discoveredDevices[device][deviceUse] =
                 ...
                 * add toolUse (resId the tool is used to extract) to toolsKnown and remove it from toolsNotKnown.
                 */
                idx = find(glob.industriesNotKnown.begin(), glob.industriesNotKnown.end(), deviceUse);
                if (idx != glob.industriesNotKnown.end()) {
                    glob.industriesNotKnown.erase(idx);
                    glob.industriesKnown.push_back(deviceUse);
                }
                break;
            case DEVFACTORY:
                glob.discoveredDevices[device][deviceUse] =
                 ...
                 * add toolUse (resId the tool is used to extract) to toolsKnown and remove it from toolsNotKnown.
                 */
                idx = find(glob.devFactoriesNotKnown.begin(), glob.devFactoriesNotKnown.end(), deviceUse);
                if (idx != glob.devFactoriesNotKnown.end()) {
                    glob.devFactoriesNotKnown.erase(idx);
                    glob.devFactoriesKnown.push_back(deviceUse);
                }
                break;
            default:
...
void Agent::makeDevice(int deviceIndex, device_name_t device)
{
    ...
    if (bestDevDev != NO_DEVICE) {
        timeUse = deviceEffortCalc(deviceIndex, device) / getDeviceFactor(bestDevDev);
        overtime += timeUse;
        ...
        devProp[device][deviceIndex].setDeviceExperience(devProp[device][deviceIndex].getDeviceExperience() +
            glob.EXPERIENCE_FOR_MAKING[device] / getDeviceFactor(bestDevDev));
    }
    else {
 ...
 * experience gained from extracting it.
 * If the resource is limited, calculate the remaining amount of each resource. 
 * \param resIndex resource index
...
void Agent::work(int resIndex, device_name_t bestDevice)
{
    ...
    /*
     * If the resource being extracted is limited, check the remaining amount. 
     * If the limited resource's amount is greater than lower bound extraction amount--MIN_RES_AMOUNT,
     * extract the resource and update the amount.
     * Else, nothing happens.
     * If the resource being extracted is unlimited, extract the resource.
     */
    if (glob.RES_AMOUNT[resIndex].first == true) {
        if (glob.RES_AMOUNT[resIndex].second > glob.MIN_RES_AMOUNT) {
            /*
            ...
            if (bestDevice != NO_DEVICE) {
                resPr.experience += 1.0 / getDeviceFactor(bestDevice);
            } else {
                resPr.experience++;
            }
            resPr.setHeld(resPr.getHeld() + 1);
            resPr.unitsExtractedToday++;
            resPr.idleResource = false;
            glob.RES_AMOUNT[resIndex].second--;
            /*
             * If the extracted resource is limited, in depletedRes and its amount <= RES_CONSIDERED_DEPLETED,
             * move it from nonDepletedRes to depletedRes,
             * and add it to newlyDepletedRes.
             */
            vector<int>::iterator idx = find(glob.nonDepletedRes.begin(), glob.nonDepletedRes.end(), resIndex);
            if ( (idx != glob.nonDepletedRes.end()) && 
                 (glob.RES_AMOUNT[resIndex].first = true) && 
                 (glob.RES_AMOUNT[resIndex].second <= glob.RES_CONSIDERED_DEPLETED) ) {
                glob.nonDepletedRes.erase(idx);
                glob.depletedRes.push_back(resIndex);
                glob.newlyDepletedRes.push_back(resIndex);
            }
        }
    } else {
        if (bestDevice != NO_DEVICE) {
...
/**
 * When it has completed the resource-extraction phase,
 * adjust agent resource-extraction experience,
 * pay a proportional tax for each gathered resource,
 * and update device recipe--any device that is made of, or for, a depleted resource cannot be invented anymore.
 */
void Agent::workDayEnd()
{
    ...
    for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
        ...
        /* Agent resource experience cannot exceed maxResExperience. */
        if (resPr.experience > resPr.maxResExperience) {
            resPr.experience = resPr.maxResExperience;
        }
        ...
        /* Pay for tax */
        int tax = resPr.unitsExtractedToday * glob.TAX_RATE / (100.0);
        assert(tax <= resPr.getHeld());
        resPr.setHeld(resPr.getHeld() - tax);
    }
}
...
void Agent::endDayChecks()
{
    for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
        for (int dev = TOOL; dev <= DEVFACTORY; dev++) {
            ...
            assert( (thisDevice.getDeviceExperience() <= thisDevice.maxDeviceExperience) ||
...
/**
 * Each unit of each resource that have been gathered by agents has a chance of decaying based off of
...
void Agent::decay()
{
    for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
         ...
         */
        device_name_t deviceTypes[] = { TOOL, MACHINE, FACTORY, INDUSTRY, DEVMACHINE, DEVFACTORY };
        for (int dev = TOOL; dev <= DEVFACTORY; dev++) {
            DevProperties &thisDevice = devProp[dev][resId];
            ...
            if (thisDevice.deviceHeld > glob.DAILY_DEVICE_DECAY) {
                thisDevice.deviceHeld -= glob.DAILY_DEVICE_DECAY *
                    (thisDevice.deviceHeld / getDeviceLifetime(deviceTypes[dev]));
...
void Agent::updateDeviceComponentExperience()
{
    ...
    for (int idx = 0; idx < 5; idx++) {	// 5 items in deviceTypes[]
        ...
        for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
            ...
            /*
             * A device is set to null if its components has depleted resources or lower order devices made of depleted resources.
             * But each agent's experience is not set to 0 after the device is set to null 
             * because an agent still has that device in hand.
             * As long as deviceHeld is not 0, experience cannot be 0.
             * So need to make sure the device is not null.
             */
            if ( (devProp[type][resId].getDeviceExperience() > 0) && (glob.discoveredDevices[type][resId] != NULL) ) {
...
{
/*    double gainOverDeviceLife = 0.0;
    if (glob.discoveredDevices[device][resIndex] != NULL) {
        gainOverDeviceLife =
...
{
    if (bestDevDevice != NO_DEVICE) {
        setAsideTime -= deviceEffortCalc(toolIndex, TOOL) / getDeviceFactor(bestDevDevice);
    } else {
...
void Agent::getBackDeviceComponents(device_name_t device, int deviceIndex)
{
    ...
    if (device == TOOL) {
        ...
        if (bestDevDev != NO_DEVICE) {
            setAsideTime -= deviceEffortCalc(deviceIndex, device) / getDeviceFactor(bestDevDev);
        } else {
...
MaxInfo Agent::calcMaxDeviceGainAndIndex(device_name_t deviceType)
{
    ...
    for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
        ...
        if ( (devProp[deviceType][resId].getDeviceExperience() >= 1.0) && 
             (glob.discoveredDevices[deviceType][resId] != NULL) ) {
...
void Agent::personalDevices(device_name_t device)
{
    ...
    while (maxGain > 0) {
        ...
        double constructionTime = consideredDevice[maxIdxOffset]->expectedConstructionTime(*this);
        double payment = costOfMax - constructionTime * endDayGPM;
...
void Agent::resetTodayStats()
{
    for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
        ...
        resProp[resId].unitsExtractedToday = 0;
        for (int type = DEVMACHINE; type <= DEVFACTORY; type++) {
...
void Agent::logAgentData()
{
    ...
    for (int i = 0; i < glob.NUM_RESOURCES; i++) {
        ...
        sprintf(ostr, "%2d\t%8.4f\t%3d\t%d\t%8.4f\t%4d\t%4d", i,
                ...
                resProp[i].unitsGatheredToday, resProp[i].unitsExtractedToday);
        LOG(5) << ostr;
    }
    LOG(5) << " --- devProps --- ";

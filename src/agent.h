...
//    double patent;        //!< not used yet; for monopoly.
    ...
    double getDeviceLifetime(device_name_t dev);
    double getDeviceFactor(device_name_t dev);
};
...
/**
 * device lifetime for each device only depends on dev type, not on resId. 
 */
inline double Agent::getDeviceLifetime(device_name_t dev) {
    if (dev == TOOL) {
        return glob.TOOL_LIFETIME;
    } else if (dev == MACHINE) {
        return glob.MACHINE_LIFETIME;
    } else if (dev == FACTORY) {
        return glob.FACTORY_LIFETIME;
    } else if (dev == INDUSTRY) {
        return glob.INDUSTRY_LIFETIME;
    } else if (dev == DEVMACHINE) {
        return glob.DEV_MACHINE_LIFETIME;
    } else if (dev == DEVFACTORY) {
        return glob.DEV_FACTORY_LIFETIME;
    }
}

/**
 * deviceFactor for each device only depends on dev type, not on resId. 
 */
inline double Agent::getDeviceFactor(device_name_t dev) {
    if (dev == TOOL) {
        return glob.TOOL_FACTOR;
    } else if (dev == MACHINE) {
        return glob.MACHINE_FACTOR;
    } else if (dev == FACTORY) {
        return glob.FACTORY_FACTOR;
    } else if (dev == INDUSTRY) {
        return glob.INDUSTRY_FACTOR;
    } else if (dev == DEVMACHINE) {
        return glob.DEV_MACHINE_FACTOR;
    } else if (dev == DEVFACTORY) {
        return glob.DEV_FACTORY_FACTOR;
    }
}

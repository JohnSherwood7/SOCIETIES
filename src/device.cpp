...
{
    vector<int>::iterator idx = find(glob.depletedRes.begin(), glob.depletedRes.end(), use);
    if ((! wantsToConsiderDevDevice(agent)) || (! glob.res[use].inSimulation) ||
        (glob.discoveredDevices[type][use] == NULL) || (idx != glob.depletedRes.end()) ) {
        return 0;
    } else {
...
double Tool::gainOverLifetime(Agent &agent)
{
    double utilTool = 0.0;
    vector<int>::iterator idx = find(glob.depletedRes.begin(), glob.depletedRes.end(), use);
    if ( (glob.discoveredDevices[type][use] == NULL) || (idx != glob.depletedRes.end()) ) {
        utilTool = 0;
    } else if (agent.deviceHeldForRes(use) > glob.DAYS_OF_DEVICE_TO_HOLD * glob.DAY_LENGTH ||
        ...
        if (machineHeld > 0) {
            machineFactor = glob.MACHINE_FACTOR;
        }
        if (factoryHeld > 0) {
            factoryFactor = glob.FACTORY_FACTOR;
        }
        if (industryHeld > 0) {
            industryFactor = glob.INDUSTRY_FACTOR;
        }
        double deviceUseTime = 0.0;
        int unitsMade = 0;
...
double Factory::gainOverLifetime(Agent &agent)
{
    ...
    if (agent.deviceHeldForRes(use) > glob.DAYS_OF_DEVICE_TO_HOLD * glob.DAY_LENGTH ||
        (! glob.res[use].inSimulation) ||
        (glob.discoveredDevices[type][use] == NULL) ||
        (idx != glob.depletedRes.end())) {
        utilFactory = 0;
    } else if (agent.devProp[type][use].gainOverDeviceLifeMemoryValid) {

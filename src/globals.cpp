...
void Globals::initializeAgents()
{
     ...
     * Each non comment line has 16 values in it:
     *   amount, steepness, scaling, renewalRate, minResEff, maxResEff, maxResExp, minDevEff, maxDevEff, maxDevExp, lifetime, resTradePower, devTradePower, inventSpeed, group
     ...
     */
    RES_CONSIDERED_DEPLETED = RES_CONSIDERED_DEPLETED * MIN_RES_AMOUNT;
    NUM_AGENTS_IN_GROUP = vector<int>(NUM_AGENT_GROUPS, 0);
    NUM_ACTIVE_AGENTS_IN_GROUP = vector<int>(NUM_AGENT_GROUPS, 0);
    agent.clear();
    depletedRes = vector<int>();
    nonDepletedRes = vector<int>();
    for (int aId = 0; aId < NUM_AGENTS; aId++) {
        ...
        for (int resId = 0; resId < NUM_RESOURCES; ) {
            ...
            /* read in a line of 16 values for each resource, comma-separated. */
            vector<double> t(16, 0.0);
            for (int i = 0; i < 16; i++) {
                sline >> t[i];
                if (sline.peek() == ',') {
                    char comma;
                ...
                /*
                 * Record the initial amount of each resource.
                 * Each resource's amount is the same for all agents, so only read the values from the first agent.
                 * If amount=0, the resource does not exist. If amount<0, the resource is unlimited.
                 */
                if ((aId == 0) && (i == 0)) {
                    bool limited = true;
                    if (t[i] < 0) {
                    ...
                    INIT_RES_AMOUNT.push_back(t[i]);
                    long int temp = boost::lexical_cast<long int>(t[i]);
                    pair<bool, long int> amount = make_pair(limited, temp);
                    RES_AMOUNT.push_back(amount);
                    // Save not depleted resources into NON_DEPLETED_RES, depleted into DEPLETED_RES.
                    if ( (limited == true) && (temp <= RES_CONSIDERED_DEPLETED) ) {
                        depletedRes.push_back(resId);
                    } else {
                        nonDepletedRes.push_back(resId);
                    }
                }
            }
    ...
    /*
     * Assume all resources' initial amount are the same.
     * RES_CONSIDERED_RENEWED = the number entered in config file * initial amount
     * So only need to compare RES_AOUNT[resId].second with RES_CONSIDERED_RENEWED without checking if the resource is renewable
     */
    if (RES_AMOUNT[0].first == false) {
        RES_CONSIDERED_RENEWED = -2;    
    } else {
        RES_CONSIDERED_RENEWED = RES_CONSIDERED_RENEWED * RES_AMOUNT[0].second;
    }
...
void Globals::setAdvancedOptions()
{
    ...
    if (! END_SAVE) {
        ...
        if (simTitle == "default") {	// sim title wasn't explicity set.
            ...
            createDirectory(SAVE_FOLDER);
            if (SAVE_FOLDER != "") {
                createDirectory(SAVE_FOLDER + "/configFile");
            }
    ...
    /*
     * Check if needs to save 4 csv files for the use of database.
     */
    if (saveDatabaseSet) {
        createDirectory(saveDatabaseFolder);
    }
...
void Globals::perRunInitialization()
{
    ...
    toolsKnown = vector<int>();
    machinesKnown = vector<int>();
    factoriesKnown = vector<int>();
    industriesKnown = vector<int>();
    devMachinesKnown = vector<int>();
    devFactoriesKnown = vector<int>();
    toolsNotKnown = vector<int>();
    machinesNotKnown = vector<int>();
    factoriesNotKnown = vector<int>();
    industriesNotKnown = vector<int>();
    devMachinesNotKnown = vector<int>();
    devFactoriesNotKnown = vector<int>();
    ...
    if (START_DAY == 0) {
        for (int resId = 0; resId < NUM_RESOURCES; resId++) {
            ...
            // No device should be known
            toolsNotKnown.push_back(resId);
            machinesNotKnown.push_back(resId);
            factoriesNotKnown.push_back(resId);
            industriesNotKnown.push_back(resId);
            devMachinesNotKnown.push_back(resId);
            devFactoriesNotKnown.push_back(resId);
        }
    } else {
        /*
...
void Globals::printConfig()
{
    ...
    cout << "Number of resources is " << NUM_RESOURCES << endl;
    cout << "Number of groups/types of agents is " << NUM_AGENT_GROUPS << endl;
    cout << "Max amount of each resource is " << MAX_RES_AMOUNT << endl << endl;

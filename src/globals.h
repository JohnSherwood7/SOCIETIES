...
public:
    ...
    bool   saveDatabaseSet;         // *** initialized in parse_args()
    string saveDatabaseFolder;      // *** initialized in parse_args()
    string batchID;                 // *** initialized in parse_args()
    string iterationID;             // *** initialized in parse_args()
    int    iterationNum;            // *** initialized in parse_args()
    string heteroID;                // *** initialized in parse_args()
    ...
    /**
     * The max amount of each resource allowed in the environment.
     */
    long int MAX_RES_AMOUNT;
    /**
     * The min amount of each resource allowed in the environment.
     * A usage of social control.
     * Agents are not allowed to extract a resource if its amount is <= the min amount.
     */
    int MIN_RES_AMOUNT;

    /**
     * To record the initial amount for each resource.
     */
    vector<int> INIT_RES_AMOUNT;

    /**
     * At the end of each work day, agents need to pay for tax.
     * For each resource, an agent pays a proportional tax,
     * i.e. the tax for each resource = the amount of the resource gathered today * TAX_RATE
     */
    double TAX_RATE;                    // *** initialized in parse_args()
    ...
    /*
     * Indexed by resources Id.
     * The boolean indicates whether a resouce is limited. True -> limited. False -> unlimited.
     * long int: the current amount of a resource.
     */
    vector< pair<bool, long int> > RES_AMOUNT;    // *** initialized in initializeAgents()

    vector<int> depletedRes;                     // *** initialized in initializeAgents()
    vector<int> nonDepletedRes;                  // *** initialized in initializeAgents()
    vector<int> newlyDepletedRes;                // *** initialized in initializeGlobalStructures()
    double RES_CONSIDERED_DEPLETED;              // *** initialized in initializeAgents()
    double RES_CONSIDERED_RENEWED;               // *** initialized in initializeAgents()
    ...
    vector<int> toolsKnown;
    vector<int> machinesKnown;
    vector<int> factoriesKnown;
    vector<int> industriesKnown;
    vector<int> devMachinesKnown;
    vector<int> devFactoriesKnown;
    vector<int> toolsNotKnown;
    vector<int> machinesNotKnown;
    vector<int> factoriesNotKnown;
    vector<int> industriesNotKnown;
    vector<int> devMachinesNotKnown;
    vector<int> devFactoriesNotKnown;
...
//	this is used throughout the program, primarily in statstracker to serve as a place holder that will get filled eventually.
    ...
    vector<vector<vector<int> > > EMPTY_VECTOR_OF_VECTORS_OF_VECTORS_OF_INTS;

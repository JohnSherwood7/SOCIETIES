...
ResProperties::ResProperties()
{
    ...
    /*
     * The renewal rate for resources.
     * in percent: the units of a resource reproduced/regenerated each day by 100 existing resource units
     *     --the units of the resource in the environment
     * If it's 0, then the resource is nonrenewable.
     */
    renewalRate = 0.0;
}
 ...
 * 
 * BRH 6.25.2014
 * The resource and device effort curves are based on Gompertz (sigmoid) functions:
 * Y(X) = A*exp(-B * exp(-C * X)),
 * where A is the upper asymptote, B translates, C scales it.
 * Here is a plot of the resource effort curve using default values:
 * http://fooplot.com/plot/l42er5sa1d
 */

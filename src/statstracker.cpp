...
ProductionStats::ProductionStats()
{
    ...
    for (int type = TOOL; type < glob.getNumDeviceTypes(); type++) {
        ...
        recipe.push_back(glob.EMPTY_VECTOR_OF_VECTORS_OF_VECTORS_OF_INTS);
        ...
        for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
            ...
            recipe[type].push_back(glob.EMPTY_VECTOR_OF_VECTORS_OF_INTS);
        }
...
/**
 * Record the recipe for each discovered device.
 * If a discovered device does not exist anymore, delete the recipe.
 */
void ProductionStats::calcRecipe()
{
    vector<int> comp;
    for (int type = 0; type < glob.getNumDeviceTypes(); type++) {
        for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
            if ( (glob.discoveredDevices[type][resId] != NULL) && (glob.discoveredDevices[type][resId]->agentsKnown() > 0) ) {
                comp = glob.discoveredDevices[type][resId]->components;
            } else {
                comp = vector<int>();
            }
            recipe[type][resId].push_back(comp);
        }
    }
}
    ...
    for (int type = DEVMACHINE; type <= DEVFACTORY; type++) {
        ...
        for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
            ...
            for (int aId = 0; aId < glob.NUM_AGENTS; aId++) {
                ...
                devMadeWithDevice +=
                    glob.agent[aId]->devicesMadeWithDevDevicesToday[type][resId];
                temp[glob.agent[aId]->group][type] += glob.agent[aId]->devicesMadeWithDevDevicesToday[type][resId];
...
OtherStats::OtherStats()
{
    ...
    for (int i = 0; i < glob.NUM_RESOURCES; i++) {
        resAmount.push_back(vector<long int>());
    }
}
...
void OtherStats::dailyUpdate()
{
    ...
    for (int i = 0; i < glob.NUM_RESOURCES; i++) {
        resAmount[i].push_back(glob.RES_AMOUNT[i].second);
    }
    depletedRes.push_back(glob.depletedRes.size());
...
void OtherStats::calcNumDevicesInvented()
{
    ...
    for (int type = 0; type < glob.getNumDeviceTypes(); type++) {
        ...
        numberOfInventedDevices[type].push_back(knownDevices);
    }
...
/**
 * Calculate the the ratio of the current total amount of all resources in the environment
 * to the total initial amount of all resources.
 * Assume resource homogeneity.
 */
void OtherStats::calcEnvironment()
{
    /*
     * If res0's initial amount is -1, 
     * then all resources are unlimited.
     * push 1 into environment.
     */
    if (glob.INIT_RES_AMOUNT[0] == -1) {
        environment.push_back(1.0);
    } else {
        int initialTotal = 0;         // the total initial amount of all unlimited resources
        long int sum = 0;
        for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
            initialTotal += glob.INIT_RES_AMOUNT[resId];
        }
        for (int resId = 0; resId < glob.NUM_RESOURCES; resId++) {
            sum += glob.RES_AMOUNT[resId].second;
        }
        environment.push_back(double(sum)/double(initialTotal));
    }
}

...
class ProductionStats
{
    ...
    /**
     * The number of all resources gathered by each agent on each day.
     * indexed by agentId, then day
     */
    vector< vector<int> > resGatheredByAgent; 	        
    /**
     * The number of all resources gathered by all agents in each group on each day.
     * indexed by groupId, then day
     */
    vector< vector<int> > resGatheredByGroup;
    ...
    /* The recipe for each device on each day. */
    vector< vector< vector< vector<int> > > > recipe; 
...
public:
    ...
    vector< vector< vector< vector<int> > > > getRecipe() { return recipe; }; 
...
class OtherStats
{
    ...
    /**
     * The amount of each resource in the environment.
     * If any of the resource is unlimited, record it as -1.
     * Indexed by resID then by day.
     */
    vector< vector<long int> > resAmount;

    /**
     * The number of depleted resources, i.e.
     * resources whose amount <= RES_CONSIDERED_DEPLETED.
     */
    vector<int> depletedRes;

    /**
     * The ratio of current total amount of resources in the environment
     * to the total initial amount.
     */
    vector<double> environment;
...
public:
    ...
    vector< vector<long int> > getResAmount() { return resAmount; };
    vector<int> getDepletedRes() { return depletedRes; };
    vector<double> getEnvironment() { return environment; };
